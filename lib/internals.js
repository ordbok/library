...
var ordbok_core_1 = require("./ordbok-core");
var Internals;
(function (Internals) {
    ...
    function assembleFiles(sourceFolder, targetFolder, config) {
        ...
        getFiles(sourceFolder, /\.(?:md|markdown)$/).forEach(function (sourceFile) {
            var markdown = new ordbok_core_1.Markdown(FS.readFileSync(sourceFile).toString());
            plugins.forEach(function (plugin) {
            ...
            markdown.pages.forEach(function (markdownPage, pageIndex) {
                plugins.forEach(function (plugin) {
                    return plugin.onWriteFile &&
                        plugin.onWriteFile(Path.join(targetFolder, (ordbok_core_1.Utilities.getBaseName(sourceFile) +
                            ordbok_core_1.Dictionary.FILE_SEPARATOR +
